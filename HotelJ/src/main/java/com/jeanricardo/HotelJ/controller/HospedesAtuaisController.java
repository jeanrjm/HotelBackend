...
@RestController
public class HospedesAtuaisController {
//localhost:8080/hospedes/atuais

    @GetMapping("hospedes/atuais")
    public ResponseEntity<String>index() throws Exception {
        
        String resp="";
        try {
            ArrayList<Hospede> hospedes = new Hotel().getHospedesAtuais();
            if(hospedes==null || hospedes.size()==0){
                return new ResponseEntity<>("Sem hospedes atuais", HttpStatus.OK);
            }else{
